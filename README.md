...
- SRC350-M2/M3はFOUP・Reticle、HU300-M3はFOUPを選択可能。HU300-M3は基本型式として型式固有値を持たない
- SRC350はSRC350-M2／FOUPを基本選択とし、搬送物固有値は塗りつぶさず赤い文字と左線で型式固有値から独立して表示
- パラメータNo.は独立したドロップダウンで選択し、名称・詳細・備考はキーワード検索
- 単位分類は上部フィルターと編集候補の両方で、現在の型式・搬送物に存在する分類だけを表示。削除済み分類は候補に残さず、表示中一覧をExcel出力
- 型式固有値のハイライトと差分フィルター
...
インポート画面の「インポート用ExcelフォーマットをDL」から、見出し例と入力ルールを含む`.xlsx`を取得できます。対象シリーズ内では`パラメータNo`を照合キーとして扱います。同じ番号があれば選択した登録先（シリーズ共通または型式固有）を上書きし、番号がなければ追加します。空白セルは「変更なし」ではなく空文字で上書きされ、パラメータNoが空白の行は読み飛ばされます。規定外の追加列は`extra`に保持します。Excelは先頭シートのみ読み込みます。
