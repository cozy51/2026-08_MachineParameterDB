...
export function App() {
  ...
  return <div className={`app ${edit?`is-editing edit-scope-${editScope}`:''}`}>
    ...
    {authOpen&&<div className="modal-back" onMouseDown={()=>setAuthOpen(false)}><div className="auth-dialog cloud-settings" onMouseDown={event=>event.stopPropagation()}><button className="modal-x" onClick={()=>setAuthOpen(false)}><X/></button><div className="settings-title"><CloudCog/><div><h2>クラウドバックアップ設定</h2><p>Supabase Storageへの接続とログインを設定します。</p></div></div>{!supabase?<><div className="setup-status missing"><AlertTriangle/><div><b>環境変数が未設定です</b><span>次の2項目を設定してアプリを再起動してください。</span></div></div><div className="env-guide"><code>VITE_SUPABASE_URL</code><code>VITE_SUPABASE_ANON_KEY</code></div><ol><li>プロジェクト直下の <code>.env.example</code> を <code>.env.local</code> にコピー</li><li>Supabase DashboardのURLとAnon Keyを入力</li><li>開発サーバーまたはアプリを再起動</li></ol><p className="settings-note">ブラウザへ秘密鍵を保存しないため、設定値はこの画面から直接入力できません。Service Role Keyは使用しないでください。</p></>:<><div className="setup-status ready"><Check/><div><b>Supabase接続設定済み</b><span>ログインするとユーザー専用バックアップを利用できます。</span></div></div>{cloudError&&<div className="import-error">{cloudError}</div>}<label>メールアドレス<input type="email" value={email} onChange={event=>setEmail(event.target.value)}/></label><label>パスワード<input type="password" value={password} onChange={event=>setPassword(event.target.value)}/></label><button className="auth-submit" disabled={!email||!password} onClick={login}>ログイン</button></>}</div></div>}
  </div>
}
