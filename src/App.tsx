...
const searchHighlightKeys: (keyof Parameter)[] = ['number','name','detail','note'];
...
export function App() {
  ...
  const rows=useMemo(()=>resolvedRows.filter(p=>(!onlyDiff||p.changedFields.length>0)&&(!parameterNumber||p.number===parameterNumber)&&(!unitCategoryFilter||p.unitCategory===unitCategoryFilter)&&(!query||[p.number,p.name,p.detail,p.note].join(' ').toLowerCase().includes(query.toLowerCase()))),[resolvedRows,parameterNumber,unitCategoryFilter,query,onlyDiff]);
