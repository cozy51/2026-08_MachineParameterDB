...
export function App() {
  ...
  function downloadImportTemplate(){
    const headers=['パラメータNo','標準的な値','単位','単位分類','パラメータ名称','設定値詳細','備考'];const sheet=XLSX.utils.aoa_to_sheet([headers,['7001','0','—','選択値','パラメータ名称の例','複数行の説明も入力できます','任意']]);sheet['!cols']=[{wch:16},{wch:14},{wch:10},{wch:14},{wch:28},{wch:60},{wch:28}];
    const guide=XLSX.utils.aoa_to_sheet([['インポート仕様'],['主キー','パラメータNo（対象シリーズ内で照合）'],['既存番号','選択した登録先の値で上書き'],['新規番号','新しいパラメータとして追加'],['空白セル','空文字として登録されるため、既存値も空白で上書き'],['パラメータNoが空白の行','読み飛ばし'],['追加列','列名と値を追加項目として保持'],['対象シート','先頭シートのみ読み込み']]);guide['!cols']=[{wch:28},{wch:64}];
    const book=XLSX.utils.book_new();XLSX.utils.book_append_sheet(book,sheet,'インポート用');XLSX.utils.book_append_sheet(book,guide,'入力ルール');XLSX.writeFile(book,'MachineParameterDB-import-template.xlsx');setToast('インポート用Excelをダウンロードしました');
  }
  ...
  return <div className={`app ${edit?`is-editing edit-scope-${editScope}`:''} ${hasCargoOverrides?'has-cargo-overrides':''}`}>
    ...
    {conflictOpen&&<div className="modal-back confirm-back" onMouseDown={()=>setConflictOpen(false)}><div className="confirm-dialog conflict-dialog" role="dialog" aria-modal="true" aria-labelledby="conflict-title" onMouseDown={event=>event.stopPropagation()}><div className="confirm-icon"><AlertTriangle/></div><h2 id="conflict-title">クラウドとの競合を解決</h2><p>別のPCで更新された可能性があるため、自動バックアップを停止しています。</p><div className="conflict-choice recommended"><b>推奨：クラウドの最新版を取得</b><span>このPCのデータをクラウドの内容へ更新します。実行前にもう一度確認します。</span><button onClick={()=>{setConflictOpen(false);restoreCloud()}}>クラウドから更新</button></div><div className="conflict-choice"><b>現在のローカルデータを確認</b><span>何も変更せず画面へ戻り、必要な内容を確認・エクスポートできます。</span><button onClick={()=>setConflictOpen(false)}>ローカルを確認する</button></div><details><summary>非推奨：クラウドをローカルデータで上書き</summary><p>クラウド側の新しい変更が失われる可能性があります。内容を確認できた場合だけ実行してください。</p><button className="danger-force" onClick={()=>setForceConfirmOpen(true)}>このPCのデータでクラウドを上書き</button></details></div></div>}
