...
import { normalizeSeries } from './storage';
...
export type UploadResult={uploaded:false;relation:'cloud-newer'|'conflict';cloud:CloudBackup}|{uploaded:true;relation:'same';cloud:CloudBackup|null;skipped?:boolean};
...
export async function downloadBackup(userId:string):Promise<CloudBackup|null>{
 ...
 const{data:list}=await supabase.storage.from(BACKUP_BUCKET).list(userId,{search:'MachineParameterDB-latest.json'});return{envelope,serverUpdatedAt:list?.find(file=>file.name==='MachineParameterDB-latest.json')?.updated_at??undefined};
}
...
// 比較用の正規形。キー順の違いを無視し、ローカル保存時に必ず掛かる normalizeSeries を
// クラウド側にも適用してから比較する。これを行わないと「保存すれば消える差」が競合と判定され、
// 「クラウドから更新」しても解消しない競合ループになる（ローカルは正規化済み・クラウドは元のまま）。
...
function comparableData(envelope:BackupEnvelope){try{return{series:normalizeSeries(envelope.data.series)}}catch{return envelope.data}}
async function digest(envelope:BackupEnvelope){const bytes=new TextEncoder().encode(JSON.stringify(canonical(comparableData(envelope))));const hash=await crypto.subtle.digest('SHA-256',bytes);return[...new Uint8Array(hash)].map(x=>x.toString(16).padStart(2,'0')).join('')}
...
export async function safeUpload(userId:string,local:BackupEnvelope,force=false):Promise<UploadResult>{if(!supabase)throw new Error('Supabaseが設定されていません。');const cloud=await downloadBackup(userId);if(cloud&&!force){const relation=await compareBackups(local,cloud.envelope);if(relation==='cloud-newer'||relation==='conflict')return{uploaded:false,relation,cloud};
 // 内容が同一なら再アップロードしない。毎回上げ直すとクラウド側の更新日時が現在時刻に上書きされ、
 // 「どちらが新しいか」の判断材料が失われるため。
 if(relation==='same')return{uploaded:true,relation:'same',cloud,skipped:true}}const body=new Blob([JSON.stringify(local,null,2)],{type:'application/json'});const{error}=await supabase.storage.from(BACKUP_BUCKET).upload(backupPath(userId),body,{contentType:'application/json',upsert:true});if(error)throw error;return{uploaded:true,relation:'same' as const,cloud}}
